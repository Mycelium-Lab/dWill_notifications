...
import WillAbi from './artifacts/dWill.json' assert { type: "json" }
import ERC20 from './artifacts/ERC20.json' assert { type: "json" }
...
const ethcallProviderPolygon = new Provider()
const ethcallContractPolygon = new Contract(contractAddresses.Polygon, WillAbi.abi)
...
const ethcallProviderEthereum = new Provider()
const ethcallContractEthereum = new Contract(contractAddresses.EthereumMainnet, WillAbi.abi)
...
const ethcallProviderBinanceMainnet = new Provider()
const ethcallContractBinanceMainnet = new Contract(contractAddresses.BinanceMainnet, WillAbi.abi)
...
const ethcallProviderArbitrum = new Provider()
const ethcallContractArbitrum = new Contract(contractAddresses.ArbitrumMainnet, WillAbi.abi)
...
const ethcallProviderOptimismMainnet = new Provider()
const ethcallContractOptimismMainnet = new Contract(contractAddresses.OptimismMainnet, WillAbi.abi)
...
const ethcallProviderAvalancheMainnet = new Provider()
const ethcallContractAvalancheMainnet = new Contract(contractAddresses.AvalancheMainnet , WillAbi.abi)

ethcallProviderPolygon
    .init(providerPolygon)
    .then(async () => console.log('EthCall Polygon Provider Works'))
    .catch( err => console.log(err));

ethcallProviderEthereum
    .init(providerEthereum)
    .then(() => console.log('EthCall Ethereum Provider Works'))
    .catch( err => console.log(err));
ethcallProviderBinanceMainnet
    .init(providerBinanceMainnet)
    .then(() => console.log('EthCall BinanceMainnet Provider Works'))
    .catch( err => console.log(err));
ethcallProviderArbitrum
    .init(providerArbitrum)
    .then(() => console.log('EthCall Arbitrum Provider Works'))
    .catch( err => console.log(err));
ethcallProviderOptimismMainnet
    .init(providerOptimism)
    .then(() => console.log('EthCall Optimism Provider Works'))
    .catch( err => console.log(err));
ethcallProviderAvalancheMainnet
    .init(providerAvalanche)
    .then(() => console.log('EthCall Avalanche Provider Works'))
    .catch( err => console.log(err));
...
contractPolygon.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
contractEthereum.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
contractBinanceMainnet.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
contractArbitrum.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
contractOptimism.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
contractAvalanche.on('AddWill', async (ID,owner,heir,token,timeWhenWithdraw,amount) => {
...
cron.schedule("6 6 6 * * *", async () => {
    await remainTimeCron(contractPolygon, 'Polygon Chain', NetworkExplorers.Polygon, WillPolygon, signerPolygon, ethcallContractPolygon, ethcallProviderPolygon)
}, {
    timezone: 'Europe/Moscow'
})

cron.schedule("8 8 8 * * *", async () => {
    await remainTimeCron(contractEthereum, 'Ethereum Chain', NetworkExplorers.EthereumMainnet, WillEthereum, signerEthereum, ethcallContractEthereum, ethcallProviderEthereum)
}, {
    timezone: 'Europe/Moscow'
})

cron.schedule("10 10 10 * * *", async () => {
    await remainTimeCron(contractBinanceMainnet, 'BNB Chain', NetworkExplorers.BinanceMainnet, WillBNB, signerBinanceMainnet, ethcallContractBinanceMainnet, ethcallProviderBinanceMainnet)
}, {
    timezone: 'Europe/Moscow'
})

cron.schedule("12 12 12 * * *", async () => {
    await remainTimeCron(contractArbitrum, 'Arbitrum Chain', NetworkExplorers.ArbitrumMainnet, WillArbitrum, signerArbitrum, ethcallContractArbitrum, ethcallProviderArbitrum)
}, {
    timezone: 'Europe/Moscow'
})

cron.schedule("14 14 14 * * *", async () => {
    await remainTimeCron(contractOptimism, 'Optimism Chain', NetworkExplorers.OptimismMainnet, WillOptimism, signerOptimism, ethcallContractOptimismMainnet, ethcallProviderOptimismMainnet)
}, {
    timezone: 'Europe/Moscow'
})

cron.schedule("16 16 16 * * *", async () => {
    await remainTimeCron(contractAvalanche, 'Avalanche Chain', NetworkExplorers.AvalancheMainnet, WillAvalanche, signerAvalanche, ethcallContractAvalancheMainnet, ethcallProviderAvalancheMainnet)
}, {
    timezone: 'Europe/Moscow'
})

async function remainTimeCron(contract, network, explorer, Will, signer, ethcallContract, ethcallProvider) {
    try {
        const users = await User.find()
        for (let i = 0; i < users.length; i++) {
            const willsLength = await contract.getWillsLength(users[i].address)
            let willsCall = []
            for (let i = 0; i < willsLength; i++) {
                willsCall.push(ethcallContract.getWill(users[i].address, i))
            }
            const wills = await this.props.ethcallProvider.all(willsCall)
            for (let j = 0; j < wills.length; j++) {
                try {
                    const remaining = remainingTime(wills[j].withdrawalTime)
                    if (
